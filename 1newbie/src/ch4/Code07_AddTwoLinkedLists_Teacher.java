...
extract v outside of a loop to avoid initialize and destroy in each loop

 */
public class Code07_AddTwoLinkedLists_Teacher {
    ...
    private static ListNode addTwoLinks(ListNode head1, ListNode head2) {
        ...
        // The variable's lifetime begins at the declaration and ends when the block exits
        int v = 0;
        int carrier = 0;
        while (shortNode != null) {
            v = longNode.val + shortNode.val + carrier;
            if (v / 10 > 0) {
                carrier = 1;
            }
            longNode.val = v % 10;
